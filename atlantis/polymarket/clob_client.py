...
class LiveClobClient:
    ...
    def get_min_order_size(self, token_id: str) -> Decimal | None:
        try:
            book = self._client.get_order_book(token_id)
            min_size = book.get("min_order_size") if isinstance(book, dict) else None
            return Decimal(str(min_size)) if min_size is not None else None
        except Exception:
            return None
